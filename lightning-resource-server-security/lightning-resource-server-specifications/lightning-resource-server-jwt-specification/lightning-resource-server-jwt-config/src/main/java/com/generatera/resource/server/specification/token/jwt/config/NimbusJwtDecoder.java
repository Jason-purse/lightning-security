...
import com.generatera.resource.server.specification.token.jwt.config.converter.MappedJwtClaimSetConverter;
...
import com.generatera.resource.server.specification.token.jwt.config.exception.JwtValidationException;
...
public final class NimbusJwtDecoder implements LightningJwtDecoder {
    ...
    private LightningJwt createJwt(String token, JWT parsedJwt) {
        try {
            ...
            return LightningJwt.withTokenValue(token).headers((h) -> {
                h.putAll(headers);
            }).claims((c) -> {
                c.putAll(claims);
            }).build();
