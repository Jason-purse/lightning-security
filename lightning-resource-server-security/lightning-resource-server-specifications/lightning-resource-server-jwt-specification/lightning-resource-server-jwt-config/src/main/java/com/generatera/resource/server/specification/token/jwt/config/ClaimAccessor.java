...
import com.generatera.resource.server.specification.token.jwt.config.converter.ClaimConversionService;
...
public interface ClaimAccessor {
    ...
    default Map<String, Object> getClaimAsMap(String claim) {
        if (!this.hasClaim(claim)) {
            ...
            Object claimValue = this.getClaims().get(claim);
            Map<String, Object> convertedValue = (Map)ClaimConversionService.getSharedInstance().convert(claimValue, sourceDescriptor, targetDescriptor);
            Assert.isTrue(convertedValue != null, () -> {
    ...
    default List<String> getClaimAsStringList(String claim) {
        if (!this.hasClaim(claim)) {
            ...
            Object claimValue = this.getClaims().get(claim);
            List<String> convertedValue = (List)ClaimConversionService.getSharedInstance().convert(claimValue, sourceDescriptor, targetDescriptor);
            Assert.isTrue(convertedValue != null, () -> {
