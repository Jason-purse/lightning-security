...
import com.generatera.authorization.server.common.configuration.ext.oauth2.OAuth2ProviderExtUtils;
...
public class OAuth2ExtSecurityConfigurer extends SecurityConfigurerAdapter<DefaultSecurityFilterChain, HttpSecurity>  {
    ...
    private void oauth2CommonComponentValidation(HttpSecurity builder) {
        ProviderSettingsProvider providerSettings = OAuth2ProviderExtUtils.getProviderSettings(builder);
    ...
    private void oauth2CommonComponentFill(HttpSecurity builder) {
        ...
        JWKSource<SecurityContext> jwkSource = OAuth2ProviderExtUtils.getJwkSource(builder);
