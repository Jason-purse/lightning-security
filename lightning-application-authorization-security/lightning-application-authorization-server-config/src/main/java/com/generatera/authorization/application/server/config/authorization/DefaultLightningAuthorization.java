package com.generatera.authorization.application.server.config.authorization;

import com.generatera.authorization.application.server.config.util.DateUtil;
...
public class DefaultLightningAuthorization implements LightningAuthorization, Serializable {
    ...
    public LightningUserPrincipal getPrincipal() {
        return getAttribute(LightningAuthorization.USER_INFO_ATTRIBUTE_NAME);
    }
    ...
    public static class Token<T extends LightningToken> implements Serializable {
        ...
        public boolean isExpired() {
            return this.getToken().getExpiresAt() != null && Instant.now().isAfter(DateUtil.convertTo(this.getToken().getExpiresAt()));
        }

        public boolean isBeforeUse() {
            Instant notBefore = null;
            if (!CollectionUtils.isEmpty(this.getClaims())) {
                Object nbf = this.getClaims().get("nbf");
                notBefore = DateUtil.convertTo(nbf);
            }
